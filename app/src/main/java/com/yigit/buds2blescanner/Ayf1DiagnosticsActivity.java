...
/**
 * R177 diagnostics whose wire formats are verified against GalaxyBudsClient.
 * Fit test: request 157, result 158. Self test: request/response 171.
 * Spatial data message 194 is decoded as an observation stream only.
 */
public final class Ayf1DiagnosticsActivity extends Activity {
    ...
    private void build(){
        ...
        r.addView(label("AYF1 / R177 DIAGNOSTICS",22));
        result=label("Bağlantı bekleniyor",15);r.addView(result);
        ...
        Button unsafe=btn("ANC / LEAK / MIC PAYLOAD ARAŞTIR");unsafe.setOnClickListener(v->toast("Firmware fonksiyonları bulundu; R177 wire payloadı kaynakta doğrulanmadan gönderim yapılmıyor."));r.addView(unsafe);
        result.setPadding(8,20,8,20);r.addView(result);
    ...
    private void connect(){
        ...
        BluetoothAdapter a=((BluetoothManager)getSystemService(BLUETOOTH_SERVICE)).getAdapter();
        if(a==null){toast("Bluetooth yok");return;}
    ...
    private void append(String s){runOnUiThread(()->log.append(s).append('\n'));}
