...
public class BudsManagerActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        if (adapter != null && ready()) refreshBonded();
    }
    ...
    private void buildUi(){
        ...
        statusPanel.addView(left);statusPanel.addView(right);statusPanel.addView(caseBattery);statusPanel.addView(firmware);statusPanel.addView(ambient);statusPanel.addView(eq);statusPanel.addView(wearing);root.addView(statusPanel);
        ...
        rawStatus=text("Protocol state: idle",11);rawStatus.setTextIsSelectable(true);
        // FIX: rawStatus must have exactly one parent. Previously it was added to root and then
        // added again to logScroll, causing IllegalStateException during every startup.
        ScrollView logScroll=new ScrollView(this);logScroll.addView(rawStatus);root.addView(logScroll,new LinearLayout.LayoutParams(-1,0,1));
        setContentView(root);
    }
    ...
    @SuppressLint("MissingPermission") private void refreshBonded(){
        ...
        for(BluetoothDevice d:set){TextView row=text(safeName(d)+"\nClassic: "+d.getAddress()+"\nBond: "+bond(d),13);row.setOnClickListener(v->{selectedClassic=d;connectButton.setEnabled(true);deviceTitle.setText("SEÇİLİ: "+safeName(d));addressInfo.setText("Classic/SPP: "+d.getAddress()+"\nBLE adresi farklı olabilir; bu normaldir.");});classicList.addView(row);}
    ...
    @SuppressLint("MissingPermission") private void scanBle(){
        if(!ready())return;if(scanning){stopBle();return;}bleDevices.clear();bleList.removeAllViews();leScanner=adapter.getBluetoothLeScanner();if(leScanner==null){toast("BLE scanner yok");return;}
        scanning=true;scanButton.setText("DURDUR");connection.setText("● BLE taranıyor");leScanner.startScan(null,new ScanSettings.Builder().setScanMode(ScanSettings.SCAN_MODE_LOW_LATENCY).build(),scanCallback);main.postDelayed(this::stopBle,10000);
    }
    ...
    private final ScanCallback scanCallback=new ScanCallback(){@Override public void onScanResult(int type,ScanResult r){BluetoothDevice d=r.getDevice();String a=d.getAddress();if(!bleDevices.containsKey(a)){bleDevices.put(a,d);addBle(d,r.getRssi());}}@Override public void onScanFailed(int e){main.post(()->connection.setText("● BLE scan error "+e));}};
    @SuppressLint("MissingPermission") private void addBle(BluetoothDevice d,int rssi){TextView row=text(safeName(d)+"\nBLE: "+d.getAddress()+" | RSSI "+rssi+" dBm",13);row.setOnClickListener(v->{deviceTitle.setText("BLE: "+safeName(d));addressInfo.setText("BLE: "+d.getAddress()+"\nClassic/SPP bağlantısı için eşleşmiş cihaz listesinden aynı Buds2'yi seç.");autoSelectBondedByName(safeName(d));});bleList.addView(row);}
    @SuppressLint("MissingPermission") private void autoSelectBondedByName(String name){if(name==null)return;for(BluetoothDevice d:adapter.getBondedDevices())if(name.equalsIgnoreCase(safeName(d)) || name.toLowerCase(Locale.US).contains("buds2") && safeName(d).toLowerCase(Locale.US).contains("buds2")){selectedClassic=d;connectButton.setEnabled(true);deviceTitle.setText("SEÇİLİ: "+safeName(d));addressInfo.setText("BLE cihazı ve Classic cihazı isimle eşleştirildi.\nBLE: taramadaki adres\nClassic/SPP: "+d.getAddress());return;}}

    @SuppressLint("MissingPermission") private void connect(){
        if(selectedClassic==null){toast("Önce eşleşmiş Classic/SPP cihaz seç");return;}if(connected)return;analyzer.reset();capture.setLength(0);firstCapture=true;log("CONNECT_START model=SM-R177;classic="+selectedClassic.getAddress()+";uuid="+SPP);
        ...
        io.execute(()->{try{BluetoothSocket s;try{s=selectedClassic.createRfcommSocketToServiceRecord(SPP);}catch(Exception e){s=selectedClassic.createInsecureRfcommSocketToServiceRecord(SPP);}adapter.cancelDiscovery();s.connect();socketHolder=new BluetoothSocketHolder(s);connected=true;main.post(()->{connection.setText("● RFCOMM / SPP BAĞLI");log("RFCOMM_CONNECTED address="+selectedClassic.getAddress());});readLoop(s);}catch(Exception e){connected=false;main.post(()->{connection.setText("● RFCOMM başarısız: "+e.getClass().getSimpleName());log("CONNECT_ERROR "+e);connectButton.setEnabled(true);disconnectButton.setEnabled(false);});}});
    }
    private void readLoop(BluetoothSocket s){try{byte[] buf=new byte[1024];InputStream in=s.getInputStream();while(connected){int n=in.read(buf);if(n<0)break;byte[] chunk=Arrays.copyOf(buf,n);List<BudsProtocolAnalyzer.Frame> frames=analyzer.feed(chunk);for(BudsProtocolAnalyzer.Frame f:frames)handleFrame(f);log("RX "+hex(chunk));}}catch(Exception e){main.post(()->log("RFCOMM_READ_END "+e));}finally{connected=false;main.post(()->{connection.setText("● Bağlantı kesildi");disconnectButton.setEnabled(false);connectButton.setEnabled(true);});}}
    private void handleFrame(BudsProtocolAnalyzer.Frame f){main.post(()->{log(f.summary());applyState(f);});if(autoRespond&&(f.messageId==0x60||f.messageId==0x61||f.messageId==0x62||f.messageId==0x63)){byte[] response=makeResponse(f.bytes);io.execute(()->send(response));if(timeSync&&f.messageId==0x61)main.postDelayed(()->io.execute(()->send(makeUpdateTime())),120);}}
    private void applyState(BudsProtocolAnalyzer.Frame f){if(f.messageId==0x60&&f.payloadLength>=6){byte[] b=f.bytes;int p=4;batteryL=u8(b,p+1);batteryR=u8(b,p+2);wearingState=u8(b,p+5);}if(f.messageId==0x61&&f.payloadLength>=12){byte[] b=f.bytes;int p=4;batteryL=u8(b,p+2);batteryR=u8(b,p+3);wearingState=u8(b,p+6);ambientState=u8(b,p+7);eqState=u8(b,p+10);}if(f.messageId==0x63)firmware.setText("Firmware/raw: "+hexRange(f.bytes,4,f.bytes.length-3));left.setText("Sol: "+pct(batteryL));right.setText("Sağ: "+pct(batteryR));caseBattery.setText("Kutu: Buds2 SPP durum mesajında yok / ayrı family");ambient.setText("ANC/Ambient: "+ambientName(ambientState));eq.setText("EQ: "+eqName(eqState));wearing.setText("Takılılık: "+wearingName(wearingState));}
    private String pct(int x){return x<0?"—":x+"%";}private String ambientName(int x){if(x<0)return "—";return x==1?"Açık":"Kapalı";}private String eqName(int x){if(x<0)return "—";String[] n={"Custom/Disabled","Bass boost","Soft","Dynamic","Clear","Treble"};return x<n.length?n[x]:"Mode "+x;}private String wearingName(int x){switch(x){case 0:return "Yok";case 1:return "Sol";case 16:return "Sağ";case 17:return "İkisi";default:return "0x"+Integer.toHexString(x);}}
    private byte[] makeResponse(byte[] request){if(request==null||request.length<8)return new byte[0];byte[] out=Arrays.copyOf(request,request.length);int h=(out[1]&255)|((out[2]&255)<<8);h|=0x1000;out[1]=(byte)h;out[2]=(byte)(h>>8);int crcPos=out.length-3;int crc=crc16(out,3,crcPos-3);out[crcPos]=(byte)crc;out[crcPos+1]=(byte)(crc>>8);out[out.length-1]=(byte)0xDD;return out;}
    private byte[] makeUpdateTime(){byte[] payload=new byte[12];long ms=System.currentTimeMillis();long tz=TimeZone.getDefault().getOffset(ms);ByteBuffer.wrap(payload).order(ByteOrder.LITTLE_ENDIAN).putLong(ms).putInt((int)tz);return makeFrame(0xA7,payload);}
    private byte[] makeFrame(int id,byte[] payload){int declared=1+payload.length+2,total=declared+4;byte[] b=new byte[total];b[0]=(byte)0xFD;b[1]=(byte)declared;b[2]=(byte)(declared>>8);b[3]=(byte)id;System.arraycopy(payload,0,b,4,payload.length);int crc=crc16(b,3,payload.length+1);int p=total-3;b[p]=(byte)crc;b[p+1]=(byte)(crc>>8);b[p+2]=(byte)0xDD;return b;}
    ...
    private int u8(byte[] b,int i){return b[i]&255;}private String hex(byte[] b){if(b==null)return "";StringBuilder s=new StringBuilder();for(byte x:b){if(s.length()>0)s.append(' ');s.append(String.format(Locale.US,"%02X",x&255));}return s.toString();}private String hexRange(byte[] b,int a,int z){return hex(Arrays.copyOfRange(b,a,z));}
    @SuppressLint("MissingPermission")private String safeName(BluetoothDevice d){try{String n=d.getName();return n==null?"Unknown":n;}catch(Exception e){return "Unknown";}}@SuppressLint("MissingPermission")private String bond(BluetoothDevice d){try{int x=d.getBondState();return x==BluetoothDevice.BOND_BONDED?"BONDED":x==BluetoothDevice.BOND_BONDING?"BONDING":"NONE";}catch(Exception e){return "?";}}
    ...
    private static final class BluetoothSocketHolder{final BluetoothSocket socket;BluetoothSocketHolder(BluetoothSocket s){socket=s;}InputStream in()throws Exception{return socket.getInputStream();}OutputStream out()throws Exception{return socket.getOutputStream();}void close(){try{socket.close();}catch(Exception ignored){}}}
}
